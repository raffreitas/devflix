...
namespace Devflix.Content.Catalog.E2ETests.Consumers.Categories;

public sealed class CategoryConsumerTestFixture : CategoryTestFixtureBase
{
    ...
    public async Task PublishMessageAsync(object message)
    {
        ...
        _ = await producer.ProduceAsync(
            ...
            new Message<string, string> { Key = Guid.NewGuid().ToString(), Value = rawMessage });
    }
    ...
    public MessageModel<CategoryPayloadModel> BuildValidMessage(string operation, CategoryModel categoryModel)
    {
        ...
        {
            Payload = new MessageModelPayload<CategoryPayloadModel> { Op = operation, }
        };
