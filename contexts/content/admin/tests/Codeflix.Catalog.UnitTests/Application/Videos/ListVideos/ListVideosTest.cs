...
using Codeflix.Catalog.Application.UseCases.Videos.ListVideos;
...
namespace Codeflix.Catalog.UnitTests.Application.Videos.ListVideos;

[Trait("Application", "ListVideos - Use Cases")]
public sealed class ListVideosTest : IClassFixture<ListVideosTestFixture>
{
    ...
    private readonly ListVideosUseCase _useCase;

    public ListVideosTest(ListVideosTestFixture fixture)
    {
        _fixture = fixture;
        _useCase = new ListVideosUseCase(
        ...
        _videoRepositoryMock.Setup(x =>
            x.Search(
                It.Is<SearchInput>(search =>
                    ...
                    search.OrderBy == input.Sort &&
                    search.Order == input.Dir),
                It.IsAny<CancellationToken>()
            )
        ).ReturnsAsync(new SearchOutput<Video>(
            input.Page,
            input.PerPage,
    ...
    {
        var (exampleVideosList, examplesCategories, exampleGenres) = _fixture.CreateExampleVideosListWithRelations();
        ...
        _genreRepositoryMock.Setup(repository => repository.GetListByIds(
            It.Is<List<Guid>>(list =>
    ...
    public async Task ListVideosWithoutRelationsDoesntCallOtherRepositories()
    {
        ...
        var input = new ListVideosInput(1, 10, "", "", SearchOrder.Asc);
        _videoRepositoryMock.Setup(x =>
            x.Search(It.Is<SearchInput>(search =>
                    search.Page == input.Page &&
                    search.PerPage == input.PerPage &&
                    search.Search == input.Search &&
                    search.OrderBy == input.Sort &&
        ...
        _categoryRepositoryMock.Verify(x =>
            x.GetListByIds(It.IsAny<List<Guid>>(), It.IsAny<CancellationToken>()), Times.Never);
